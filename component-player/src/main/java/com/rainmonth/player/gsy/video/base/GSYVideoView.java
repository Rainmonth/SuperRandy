...
import com.rainmonth.player.gsy.listener.GSYMediaPlayerListener;
...
import com.rainmonth.player.gsy.utils.NetInfoModule;

import java.io.File;
import java.util.HashMap;
import java.util.Map;

import static com.rainmonth.player.gsy.utils.CommonUtil.getTextSpeed;
...
public abstract class GSYVideoView extends GSYTextureRenderView implements GSYMediaPlayerListener {
    ...
    protected void startButtonLogic() {
        if (mVideoAllCallBack != null && (mCurrentState == CURRENT_STATE_NORMAL
            ...
            Debugger.printfLog("onClickStartIcon");
    ...
    protected void deleteCacheFileWhenError() {
        clearCurrentCache();
        Debugger.printfError("Link Or mCache Error, Please Try Again " + mOriginUrl);
        if (mCache) {
            Debugger.printfError("mCache Link " + mUrl);
        }
        mUrl = mOriginUrl;
    }
    ...
    public void onAutoCompletion() {
        ...
        if (mVideoAllCallBack != null && isCurrentMediaListener()) {
            Debugger.printfLog("onAutoComplete");
    ...
    public void onSeekComplete() {
        Debugger.printfLog("onSeekComplete");
    }
    ...
    public void onInfo(int what, int extra) {
        if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
            ...
            if (mHadPlay && mCurrentState != CURRENT_STATE_PREPARING && mCurrentState > 0)
            ...
            mNetInfoModule = new NetInfoModule(mContext.getApplicationContext(), new NetInfoModule.NetChangeListener() {
                ...
                public void changed(String state) {
                    if (!mNetSate.equals(state)) {
                        Debugger.printfError("******* change network state ******* " + state);
                        mNetChanged = true;
                    }
                    mNetSate = state;
                }
