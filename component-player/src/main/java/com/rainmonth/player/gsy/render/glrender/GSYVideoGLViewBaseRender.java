...
public abstract class GSYVideoGLViewBaseRender implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    protected int loadShader(int shaderType, String source) {
        ...
        if (shader != 0) {
            ...
            GLES20.glGetShaderiv(shader, GLES20.GL_COMPILE_STATUS,
                    compiled, 0);
            if (compiled[0] == 0) {
                Debugger.printfError("Could not compile shader " + shaderType + ":");
                Debugger.printfError(GLES20.glGetShaderInfoLog(shader));
    ...
    protected int createProgram(String vertexSource, String fragmentSource) {
        ...
        if (program != 0) {
            ...
            if (linkStatus[0] != GLES20.GL_TRUE) {
                Debugger.printfError("Could not link program: ");
                Debugger.printfError(GLES20.glGetProgramInfoLog(program));
    ...
    protected void checkGlError(final String op) {
        ...
        if ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
            Debugger.printfError(op + ": glError " + error);
            mHandler.post(new Runnable() {
                @Override
                public void run() {
